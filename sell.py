...
class sellclass:
    def __init__(self, root):
        ...
        self.root.focus_force()
         ...
         # === images =============
        self.laptop_image = ImageTk.PhotoImage(file=r"C:\Users\Ahmad\OneDrive\Desktop\TK-PROJECT\project\image6.png")
        self.lbl_laptop_image = Label(self.root, image=self.laptop_image)
        self.lbl_laptop_image.place(x=10, y=20)
        ...
        # ======title====
        title = Label(self.root, text="FOR SELL", font=("goudy old style", 15), bg="#0f4d7d", fg="white").place(
            x=50, y=100, width=1000)

        # ======content=====
        # =======row1========
        lbl_sellid = Label(self.root, text="SELL ID", font=("goudy old style", 15), bg="white").place(x=50,
                                                                                                               y=150)
        ...
        txt_sellid = Entry(self.root, textvariable=self.var_sell_id, font=("goudy old style", 15),
        ...
        cmb_utype = ttk.Combobox(self.root, textvariable=self.var_utype,
                                 values=("Admin", "Seller"), state='readonly',
                                 justify=CENTER, font=("goudy old style", 15))
        ...
        self.selltable = ttk.Treeview(pur_frame,
                                           columns=("Sell ID", "Land Type", "Area(SQ)", " Name","location",
        ...
        scrollx.config(command=self.selltable.xview)
        scrolly.config(command=self.selltable.yview)
        for col in ("Sell ID", "Land Type", "Area(SQ)", " Name","location","Area_Type", "Upper Price", "Lower Price",
                    "User Type", "Address", "Contact"):
            self.selltable.heading(col, text=col)

        self.selltable["show"] = "headings"

        for col in ("Sell ID", "Land Type", "Area(SQ)", " Name", "Area_Type", "Upper Price", "Lower Price",
                    "User Type", "Address", "Contact"):
            self.selltable.column(col, width=100)

        self.selltable.pack(fill=BOTH, expand=1)
        self.selltable.bind("<ButtonRelease-1>",self.get_data)
...
#================================================================================================================
    ...
    def show(self):
        ...
        cur = con.cursor()
        try:
           cur.execute("Select * from sell")  # Corrected table name
           rows = cur.fetchall()
           self.selltable.delete(*self.selltable.get_children())
           for row in rows:
            self.selltable.insert('', END, values=row)
    ...
    def add(self):
        ...
        cur = con.cursor()
        try:
            if self.var_sell_id.get() == "" or self.var_name.get() == "":
                messagebox.showerror("Error", "Sell ID and Name must be required", parent=self.root)
            else:
                cur.execute("SELECT * FROM sell WHERE sell_id=?", (self.var_sell_id.get(),))
                row = cur.fetchone()
                if row is not None:
                    messagebox.showerror("Error", "This Sell ID already assigned, try different", parent=self.root)
                else:
                    cur.execute("INSERT INTO sell (sell_id, Land_Type, Area_SQ,Name, location, Area_Type, "
                                "Upper_Price, Lower_Price, User_Type, Address, Contact) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                                (
                                    self.var_sell_id.get(),
                    ...
                    con.commit()
                    messagebox.showinfo("Success", "Sell Added Successfully", parent=self.root)
                    self.show()
        except Exception as ex:
            messagebox.showerror("Error", f"Error due to: {str(ex)}", parent=self.root)
    ...
    def get_data(self,ev):
        f=self.selltable.focus()
        content=(self.selltable.item(f))
        row=content['values']
        # print(row)
        self.var_sell_id.set(row[0]),
    ...
    def update(self):
        ...
        try:
            if self.var_sell_id.get() == "" or self.var_name.get() == "":
                ...
                row = cur.fetchone()
                if row is not None:
                    messagebox.showerror("Error", "Invalid Sell ID ", parent=self.root)
                else:
                    cur.execute("Update  sell set Land_Type=?, Area_SQ=?,Name=?, location=?, Area_Type=?,Upper_Price=?, Lower_Price=?, User_Type=?, Address=?, Contact=? where sell_id=?",
                                (
                                    self.var_landtype.get(),
                                    self.var_area_sq.get(),
                                    self.var_name.get(),
                    ...
                    con.commit()
                    messagebox.showinfo("Success", "Sell Update Successfully", parent=self.root)
                    self.show()
        except Exception as ex:
            messagebox.showerror("Error", f"Error due to: {str(ex)}", parent=self.root)
    ...
    def delete(self):
        ...
        cur = con.cursor()
        try:
            if self.var_sell_id.get() == "":
               messagebox.showerror("Error", "Sell ID must be required", parent=self.root)
            else:
                cur.execute("SELECT * FROM sell WHERE sell_id=?", (self.var_sell_id.get(),))
                row = cur.fetchone()
                if row is None:
                    ...
                    if op:
                        cur.execute("Delete from sell where sell_id=?", (self.var_sell_id.get(),))
                        con.commit()
                        messagebox.showinfo("Delete", "Sell Deleted Successfully", parent=self.root)
                        self.exist()
        except Exception as ex:
            messagebox.showerror("Error", f"Error due to: {str(ex)}", parent=self.root)
    
    def exist(self):
        self.var_sell_id.set(""),
    ...
    def search(self):
        ...
        try:
            if self.var_searchtype.get() == "Select":
               ...
               cur.execute("Select * from sell where " + self.var_searchtype.get() + " LIKE '%" + self.var_searchtxt.get() + "%'")
               rows = cur.fetchall()
               if len(rows) != 0:
                    self.selltable.delete(*self.selltable.get_children())
                    for row in rows:
