...
class purchaseclass:
    def __init__(self, root):
        ...
        self.root.focus_force()
         ...
         # === images =============
        self.laptop_image = ImageTk.PhotoImage(file=r"C:\Users\Ahmad\OneDrive\Desktop\TK-PROJECT\project\image3.png")
        ...
        # ======title====
        title = Label(self.root, text="TO PURCHASE", font=("goudy old style", 15), bg="#0f4d7d", fg="white").place(
            x=50, y=100, width=1000)

        # ======content=====
        # =======row1========
        lbl_purchaseid = Label(self.root, text="Purchaser ID", font=("goudy old style", 15), bg="white").place(x=50,
                                                                                                               y=150)
        ...
        txt_purchaseid = Entry(self.root, textvariable=self.var_purchase_id, font=("goudy old style", 15),
        ...
        # =======row2========
        lbl_name = Label(self.root, text="P-Name", font=("goudy old style", 15), bg="white").place(x=50, y=190)
        ...
        cmb_utype = ttk.Combobox(self.root, textvariable=self.var_utype,
                                 values=("Admin", "Purchaser"), state='readonly',
                                 justify=CENTER, font=("goudy old style", 15))
        ...
        self.purchasertable = ttk.Treeview(pur_frame,
                                           columns=("Purchaser ID", "Land Type", "Area(SQ)", "Purchaser Name","location",
        ...
        scrollx.config(command=self.purchasertable.xview)
        scrolly.config(command=self.purchasertable.yview)
        for col in ("Purchaser ID", "Land Type", "Area(SQ)", "Purchaser Name","location","Area_Type", "Upper Price", "Lower Price",
                    "User Type", "Address", "Contact"):
            self.purchasertable.heading(col, text=col)

        self.purchasertable["show"] = "headings"

        for col in ("Purchaser ID", "Land Type", "Area(SQ)", "Purchaser Name", "Area_Type", "Upper Price", "Lower Price",
                    "User Type", "Address", "Contact"):
            self.purchasertable.column(col, width=100)

        self.purchasertable.pack(fill=BOTH, expand=1)
        self.purchasertable.bind("<ButtonRelease-1>",self.get_data)
...
#================================================================================================================
    ...
    def show(self):
        ...
        cur = con.cursor()
        try:
           cur.execute("Select * from purchase")  # Corrected table name
           rows = cur.fetchall()
           self.purchasertable.delete(*self.purchasertable.get_children())
           for row in rows:
            self.purchasertable.insert('', END, values=row)
    ...
    def add(self):
        ...
        cur = con.cursor()
        try:
            if self.var_purchase_id.get() == "" or self.var_name.get() == "":
                messagebox.showerror("Error", "Purchase ID and Name must be required", parent=self.root)
            else:
                cur.execute("SELECT * FROM purchase WHERE purchase_id=?", (self.var_purchase_id.get(),))
                row = cur.fetchone()
                if row is not None:
                    messagebox.showerror("Error", "This Purchase ID already assigned, try different", parent=self.root)
                else:
                    cur.execute("INSERT INTO purchase (purchase_id, Land_Type, Area_SQ, Purchaser_Name, location, Area_Type, "
    ...
    def get_data(self,ev):
        f=self.purchasertable.focus()
        content=(self.purchasertable.item(f))
        row=content['values']
        # print(row)
        ...
        try:
            if self.var_purchase_id.get() == "" or self.var_name.get() == "":
                ...
                if row is not None:
                    ...
                    cur.execute("Update  purchase  set Land_Type=?, Area_SQ=?, Purchaser_Name=?, location=?, Area_Type=?,Upper_Price=?, Lower_Price=?, User_Type=?, Address=?, Contact=? where purchase_id=?",
                                (
                                    self.var_landtype.get(),
                                    self.var_area_sq.get(),
                                    self.var_name.get(),
                    ...
                    con.commit()
                    messagebox.showinfo("Success", "Purchase Update Successfully", parent=self.root)
                    self.show()
        except Exception as ex:
            messagebox.showerror("Error", f"Error due to: {str(ex)}", parent=self.root)
            ...
            if self.var_purchase_id.get() == "":
               messagebox.showerror("Error", "Purchase ID must be required", parent=self.root)
                ...
                if row is None:
                    ...
                    if op:
                        cur.execute("Delete from purchase where purchase_id=?", (self.var_purchase_id.get(),))
                        con.commit()
    ...
    def search(self):
        ...
        try:
            if self.var_searchtype.get() == "Select":
               ...
               cur.execute("Select * from purchase where " + self.var_searchtype.get() + " LIKE '%" + self.var_searchtxt.get() + "%'")
               rows = cur.fetchall()
               if len(rows) != 0:
                    self.purchasertable.delete(*self.purchasertable.get_children())
                    for row in rows:
